...
def isExternalUrl(link):
  return link.find("http://$") == -1 and link.find("&configPath=") == -1 and link.find("localhost") == -1 and link.find("127.0.0.1") == -1
  ...
  for file_name in file_names:
    ...
    with open(file_path, 'r') as md_file:
      for link in findDocsLinks(md_file.read()):
        ...
        if link == "https://sld.microfocus.com/mysoftware/index": continue
        if link == "https://github.com/opentext-idol/idol-tutorials": continue
        if link == "http://schemas.autonomy.com/aci/": continue
        if "idol-docker-host" in link: continue
